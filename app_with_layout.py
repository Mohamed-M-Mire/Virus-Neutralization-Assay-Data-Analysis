    ...
    ui.nav_panel("Plate Layout",
        ui.page_fluid(
            ...
            ui.input_select(
                ...
                selected="5 names per plate (auto-duplicate)",
            ),
            ui.input_text_area(
                "sample_names_text",
                "Sample names (one line per plate; comma-separated)",
                placeholder="Plate1: K1,K2,K3,K4,K5\nPlate2: A1,A2,A3,A4,A5",
                rows=7,
            ),
            ui.layout_columns(
...
def server(input, output, Session):
    ...
    def layout_preview():
        try:
            ...
            lines.append(f"Virus-only column: {spec['virus_col']} | Positive-control column: {spec['pos_col']}")
            return "\n".join(lines)
        except Exception as e:
