...
def _parse_plate_lines(text: str) -> List[List[str]]:
    """Parse sample name lines (one line per plate). Allows optional 'PlateX:' prefix."""
    lines = [ln.strip() for ln in (text or "").splitlines() if ln.strip()]
    parsed: List[List[str]] = []
    for ln in lines:
        if ":" in ln:
            ln = ln.split(":", 1)[1]
        names = [x.strip() for x in ln.split(",") if x.strip()]
        parsed.append(names)
    return parsed
...
def build_layout_spec(
    n_plates: int,
    sample_name_mode: Literal["10 names per plate (explicit)", "5 names per plate (auto-duplicate)"],
    sample_names_text: str,
    virus_col: int,
    pos_col: int,
) -> Dict:
    """Create a user-defined plate layout spec for downstream analysis."""
    if int(virus_col) == int(pos_col):
        raise ValueError("Virus-only and positive-control columns must be different (1–12).")
    if n_plates < 1:
        raise ValueError("Number of plates must be at least 1.")

    plate_lines = _parse_plate_lines(sample_names_text)
    if len(plate_lines) != int(n_plates):
        raise ValueError(f"Expected {n_plates} lines (one per plate), got {len(plate_lines)}.")

    sample_names_by_plate: Dict[int, List[str]] = {}
    for plate_i, names in enumerate(plate_lines, start=1):
        if sample_name_mode.startswith("10"):
            if len(names) != 10:
                raise ValueError(f"Plate {plate_i}: expected 10 sample names, got {len(names)}.")
            ten = names
        else:
            if len(names) != 5:
                raise ValueError(f"Plate {plate_i}: expected 5 sample names, got {len(names)}.")
            # Duplicate adjacent pairs => 10 total, matching the 10 sample columns.
            ten = [n for n in names for _ in (0, 1)]
        sample_names_by_plate[plate_i] = ten

    return {
        "plates": int(n_plates),
        "sample_names_by_plate": sample_names_by_plate,
        "virus_col": int(virus_col),  # 1–12
        "pos_col": int(pos_col),      # 1–12
    }

def read_raw_data(
    excel_path_name_dict: Dict[str, str],
    sample_dilutionFactors: pd.DataFrame,
    sample_names_duplicated: pd.DataFrame | None,
    ...
    for filename, filepath in excel_path_name_dict.items():
        ...
        try:
            ...
            # Determine control columns (0-based) from layout_spec, defaulting to the last two columns.
            if layout_spec is None:
                virus_col_idx = 10
                pos_col_idx = 11
            else:
                virus_col_idx = int(layout_spec["virus_col"]) - 1
                pos_col_idx = int(layout_spec["pos_col"]) - 1
                if virus_col_idx == pos_col_idx:
                    raise ValueError(f"{filename}: virus-only and positive-control columns must be different.")

            # Rename chosen control columns
            cols = list(result_df.columns)
            result_df.rename(
                columns={
                    cols[virus_col_idx]: "virus_cells_only",
                    cols[pos_col_idx]: "positive_ctrl",
                },
                inplace=True,
            )

            # Reorder so samples are first, controls are last (keeps downstream assumptions stable)
            sample_cols = [c for c in result_df.columns if c not in ["virus_cells_only", "positive_ctrl"]]
            result_df = result_df[sample_cols + ["virus_cells_only", "positive_ctrl"]]

            # Set dilution factor index
            result_df.index= sample_dilutionFactors["dilution_factors"]
            virus_plate_rawData[filename]= result_df

            # Assign sample column names (10) from either uploaded sample_names file (legacy) or UI layout_spec.
            if layout_spec is None:
                if sample_names_duplicated is None:
                    raise ValueError("sample_names_duplicated is required when layout_spec is None.")

                # Legacy behavior: filename suffix indicates which plate slice to apply.
                if filename.endswith("1"):
                    ...
                    raise ValueError(f"Unrecognized plate suffix for file '{filename}'. Expected suffix 1–5.")

            else:
                # UI behavior: parse plate number from trailing "-<n>"
                try:
                    plate_num = int(filename.split("-")[-1])
                except Exception as e:
                    raise ValueError(f"{filename}: could not parse plate number from filename.") from e

                if plate_num not in layout_spec["sample_names_by_plate"]:
                    raise ValueError(f"{filename}: no sample names provided for plate {plate_num}.")

                new_column_names = layout_spec["sample_names_by_plate"][plate_num]

            if len(new_column_names) != len(sample_cols):
                raise ValueError(
                    f"{filename}: expected {len(sample_cols)} sample names, got {len(new_column_names)}."
                )

            virus_plate_rawData[filename].columns = new_column_names + ["virus_cells_only", "positive_ctrl"]
...
app_ui= ui.page_navbar(
    ...
    ui.nav_panel("Plate Layout",
        ui.page_fluid(
            ui.h3("Define plate layout"),
            ui.p("Enter sample names for each plate and choose which columns are the control wells. "
                 "This layout can be used in Analyze Data when 'Use Plate Layout tab' is checked."),
            ui.input_numeric("n_plates", "Number of plates", value=2, min=1, max=5),
            ui.input_select(
                "sample_name_mode",
                "Sample naming mode",
                choices=["5 names per plate (auto-duplicate)", "10 names per plate (explicit)"],
                selected="10 names per plate (explicit)",
            ),
            ui.input_text_area(
                "sample_names_text",
                "Sample names (one line per plate; comma-separated)",
                placeholder="Plate1: K1,K1,K2,K2,K3,K3,K4,K4,K5,K5\nPlate2: K1,K1,K2,K2,K3,K3,K4,K4,K5,K5",
                value="Plate1: K1,K1,K2,K2,K3,K3,K4,K4,K5,K5\nPlate2: K1,K1,K2,K2,K3,K3,K4,K4,K5,K5",
                rows=7,
            ),
            ui.layout_columns(
                ui.input_numeric("virus_col", "Virus-only column (1–12)", value=11, min=1, max=12),
                ui.input_numeric("pos_col", "Positive-control column (1–12)", value=12, min=1, max=12),
                col_widths=[6, 6],
            ),
            ui.hr(),
            ui.h4("Layout preview"),
            ui.output_text_verbatim("layout_preview"),
        ),
    ),
        ...
        ui.layout_sidebar(
            ui.sidebar(
                ui.div(
                    ...
                    ui.input_checkbox("use_ui_layout", "Use Plate Layout tab (ignore Sample Names file)", value=False),
                    ui.div(
                        ui.tags.style("""
                        ...
                        """),
                        ui.panel_conditional("input.use_ui_layout == false",
...
def server(input, output, Session):
    ...
    @render.text
    def layout_preview():
        # Don’t show scary errors while the user is still filling things out.
        if not input.use_ui_layout():
            return "Plate Layout is optional. To use it, check: 'Use Plate Layout tab (ignore Sample Names file)' in Analyze Data."
        txt = (input.sample_names_text() or "").strip()
        if not txt:
            return ("Paste sample names above (one line per plate, comma-separated). Example:\n"
                    "Plate1: K1,K1,K2,K2,K3,K3,K4,K4,K5,K5\n"
                    "Plate2: K1,K1,K2,K2,K3,K3,K4,K4,K5,K5")
        try:
            spec = build_layout_spec(
                input.n_plates(),
                input.sample_name_mode(),
                txt,
                input.virus_col(),
                input.pos_col(),
            )
            lines = []
            for p in range(1, spec["plates"] + 1):
                lines.append(f"Plate {p}: {spec['sample_names_by_plate'][p]}")
            lines.append(
                f"Virus-only column: {spec['virus_col']} | Positive-control column: {spec['pos_col']}"
            )
            return "\n".join(lines)
        except Exception as e:
    ...
    def process_data():
        ...
        try:
            ...
            if input.use_ui_layout():
                # Dilution factors are still loaded from uploaded files.
                sample_dilutionFactors = pd.read_csv(sample_dilution_file[0]["datapath"], sep="\t")
                ctrl_dilutionFactors = pd.read_csv(ctrl_dilution_file[0]["datapath"], sep="\t")
                sample_names_duplicated = None
                layout_spec = build_layout_spec(
                    input.n_plates(),
                    input.sample_name_mode(),
                    input.sample_names_text(),
                    input.virus_col(),
                    input.pos_col(),
                )
            else:
